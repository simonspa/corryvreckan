 ...
 */
#define IFLOG(level) if(corryvreckan::LogLevel::level <= corryvreckan::Log::getReportingLevel() && !corryvreckan::Log::getStreams().empty())
...
#define LOG_PROGRESS(level, identifier)                                                                                     \
    if(corryvreckan::LogLevel::level <= corryvreckan::Log::getReportingLevel() && !corryvreckan::Log::getStreams().empty())                   \
    corryvreckan::Log().getProcessStream(                                                                                         \
        identifier, corryvreckan::LogLevel::level, __FILE_NAME__, std::string(static_cast<const char*>(__func__)), __LINE__)
