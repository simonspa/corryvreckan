  ...
  while (entry = readdir(directory)){
    ...
    if (entry->d_type == DT_DIR){
      ...
      if(m_nFiles.count(detectorID) > 0){
        ...
        if(parameters->detector.count(detectorID) == 0){LOG(ERROR) <<"Detector "<<detectorID<<" has no alignment/conditions loaded. Please check that it is in the alignment file"; return;}
...
//        if(detectorID == "W0019_F07") LOG(TRACE) <<"Updating heartbeat part 1. Now syncTime = "<<(double)m_syncTime[detectorID]/(4096. * 40000000.);
      }
...
//        if(detectorID == "W0019_F07") LOG(TRACE) <<"Updating heartbeat part 2. Now syncTime = "<<(double)m_syncTime[detectorID]/(4096. * 40000000.);
...
//      if(detectorID == "W0019_F07") LOG(TRACE) <<"Updating heartbeat. Now syncTime = "<<(double)m_syncTime[detectorID]/(4096. * 40000000.);

//       LOG(TRACE) <<"Updating heartbeat. Now syncTime = "<<(double)m_syncTime[detectorID]/(4096. * 40000000.)<<" for detector "<<detectorID;
    }
...
//          LOG(DEBUG) <<"Signal has a time beyond the current event: "<<(double)time/(4096. * 40000000.);
          break;
        }
        ...
        LOG(DEBUG) <<"Power is "<< (powerOn ? "on" : "off") <<" power! Time: "<<std::setprecision(10)<<(double)time/(4096. * 40000000.);
        ...
        if(!shutterClosed){
          ...
          m_shutterOpen = true;
          LOG(TRACE) <<"Have opened shutter with signal "<<shutterSignal->type()<<" at time "<<(double)time/(4096. * 40000000.);
        }

        if(shutterClosed && m_shutterOpen){
          spidrData->push_back(shutterSignal);
          m_shutterOpen = false;
          LOG(TRACE) <<"Have closed shutter with signal "<<shutterSignal->type()<<" at time "<<(double)time/(4096. * 40000000.);
        }

        LOG(DEBUG) <<"Shutter is "<< (shutterClosed ? "closed" : "open") <<". Time: "<<std::setprecision(10)<<(double)time/(4096. * 40000000.);
      ...
      if(header2 == 0x6){
        ...
        LOG(DEBUG) <<"Turned on power! Time: "<<(double)time/(4096. * 40000000.);
      }
      // 0x7 is power off
      if(header2 == 0x7){
        const uint64_t time( (pixdata & 0x0000000FFFFFFFFF) << 12 );
        ...
        spidrData->push_back(signal);
        LOG(DEBUG) <<"Turned off power! Time: "<<(double)time/(4096. * 40000000.);
      }
       */
    }
    ...
    if(header == 0xA || header == 0xB) {
      ...
      //LOG(DEBUG) <<"Sync time "<<(double)m_syncTime[detectorID]/(4096. * 40000000.);
