#ifndef TIMEPIX1TRACK_H
#define TIMEPIX1TRACK_H 1
...
class Timepix1Track : public TestBeamObject {
...
public:
  ...
  Timepix1Track(Timepix1Track* track){
    ...
    for(int iTrackCluster=0; iTrackCluster<trackClusters.size(); iTrackCluster++){
      ...
      Timepix1Cluster* cluster = new Timepix1Cluster(trackCluster);
      m_trackClusters.push_back(cluster);
    }
    Timepix1Clusters associatedClusters = track->associatedClusters();
    for(int iAssociatedCluster=0; iAssociatedCluster<associatedClusters.size(); iAssociatedCluster++){
      Timepix1Cluster* associatedCluster = associatedClusters[iAssociatedCluster];
      Timepix1Cluster* cluster = new Timepix1Cluster(associatedCluster);
  ...
  void addCluster(Timepix1Cluster* cluster){
    m_trackClusters.push_back(cluster);
  }
  void addAssociatedCluster(Timepix1Cluster* cluster){
    m_associatedClusters.push_back(cluster);
  }
  ...
  void fit(){
    ...
    for(int iCluster=0;iCluster<nClusters;iCluster++){
      // Get the cluster
      Timepix1Cluster* cluster = m_trackClusters[iCluster];
  ...
  double distance2(Timepix1Cluster* cluster){
  ...
  Timepix1Clusters clusters(){return m_trackClusters;}
  Timepix1Clusters associatedClusters(){return m_associatedClusters;}
  ...
  // Member variables
  Timepix1Clusters m_trackClusters;
  Timepix1Clusters m_associatedClusters;
  ...
  ClassDef(Timepix1Track,1)

};

// Vector type declaration
typedef std::vector<Timepix1Track*> Timepix1Tracks;

#endif // TIMEPIX1TRACK_H
