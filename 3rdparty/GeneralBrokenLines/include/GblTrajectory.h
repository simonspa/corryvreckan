...
namespace gbl {
    ...
    public:
        ...
        /**
         * Composed of curved trajectories in space. The precision matrix for the external measurements is specified as
         * matrix.
         *
        ...
        unsigned int
        fit(double& Chi2, int& Ndf, double& lostWeight, const std::string& optionList = "", unsigned int aLabel = 0);
    ...
    GblTrajectory::GblTrajectory(const std::vector<std::pair<std::vector<GblPoint>, Eigen::MatrixXd>>& aPointsAndTransList,
        ...
        static_assert(static_cast<int>(Measurements::RowsAtCompileTime) == static_cast<int>(Derivatives::RowsAtCompileTime),
                      "GblTrajectory: rows(Measurements) and rows(Derivatives) must be equal");
        static_assert(static_cast<int>(Measurements::RowsAtCompileTime) == static_cast<int>(Precision::RowsAtCompileTime),
