...
namespace gbl {
     ...
     */
    void GblData::addDerivatives(const std::vector<unsigned int>& index, const std::vector<double>& derivatives) {
    ...
    void GblData::setPrediction(const VVector& aVector) {
        ...
        if(theNumLocal > 0) {
            ...
            for(unsigned int i = 0; i < moreDerivatives.size(); ++i) {
                thePrediction += moreDerivatives[i] * aVector(moreParameters[i] - 1);
            }
        }
    }
    ...
    double GblData::getChi2() const {
        ...
        {
            if(scaledResidual < 4.6851) {
                chi2 = (1.0 - pow(1.0 - 0.045558 * scaledResidual * scaledResidual, 3)) / (3. * 0.045558);
            } else {
                chi2 = 1.0 / (3. * 0.045558);
            }
     ...
     */
    unsigned int GblData::getLabel() const { return theLabel; }

    /// Get type.
    /**
     * \return type
     */
    dataBlockType GblData::getType() const { return theType; }
     ...
     */
    void GblData::getLocalData(
        double& aValue, double& aWeight, unsigned int& numLocal, unsigned int*& indLocal, double*& derLocal) {
