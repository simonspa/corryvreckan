...
namespace gbl {
     ...
     * \param [in] minPrecision Minimal precision to accept measurement
     */
    GblMeasurement::GblMeasurement(const TVectorD& aResiduals, const TVectorD& aPrecision, double minPrecision) {
        enabled = true;
        measDim = aResiduals.GetNrows();
        measPrecMin = minPrecision;
        transFlag = false;
     ...
     * \param [in] minPrecision Minimal precision to accept measurement
     */
    GblMeasurement::GblMeasurement(const TVectorD& aResiduals, const TMatrixDSym& aPrecision, double minPrecision) {
        enabled = true;
        measDim = aResiduals.GetNrows();
        measPrecMin = minPrecision;
        TMatrixDSymEigen measEigen(aPrecision);
     ...
     */
    void GblMeasurement::setEnabled(bool flag) { enabled = flag; }

    /// Get enabled flag.
    /**
     * \return enabled flag
     */
    bool GblMeasurement::isEnabled() const { return enabled; }
     ...
     */
    unsigned int GblMeasurement::getMeasDim() const { return measDim; }

    /// get precision cutoff.
    /**
     * \return minimal measurement precision (for usage)
     */
    double GblMeasurement::getMeasPrecMin() const { return measPrecMin; }
     ...
     */
    void GblMeasurement::getMeasurement(Matrix5d& aProjection, Vector5d& aResiduals, Vector5d& aPrecision) const {
        aProjection.bottomRightCorner(measDim, measDim) = measProjection.bottomRightCorner(measDim, measDim);
    ...
    unsigned int GblMeasurement::getNumLocals() const { return localDerivatives.cols(); }

    /// Retrieve local derivatives from a point.
    const MatrixXd& GblMeasurement::getLocalDerivatives() const { return localDerivatives; }

    /// Retrieve number of global derivatives from a point.
    unsigned int GblMeasurement::getNumGlobals() const { return globalDerivatives.cols(); }

    /// Retrieve global derivatives labels from a point.
    /**
     * \param [out] aLabels Global labels
     */
    void GblMeasurement::getGlobalLabels(std::vector<int>& aLabels) const { aLabels = globalLabels; }

    /// Retrieve global derivatives from a point.
    /**
     * \param [out] aDerivatives  Global derivatives
     */
    void GblMeasurement::getGlobalDerivatives(MatrixXd& aDerivatives) const { aDerivatives = globalDerivatives; }
    ...
    void GblMeasurement::printMeasurement(unsigned int level) const {
        ...
        if(level > 0) {
            ...
            if(measDim) {
                std::cout << "   Projection: " << std::endl << measProjection.format(CleanFmt) << std::endl;
                std::cout << "   Residuals: " << measResiduals.transpose().format(CleanFmt) << std::endl;
                std::cout << "   Precision (min.: " << measPrecMin << "): " << measPrecision.transpose().format(CleanFmt)
                          << std::endl;
            }
            if(localDerivatives.cols()) {
                std::cout << "  Local Derivatives:" << std::endl << localDerivatives.format(CleanFmt) << std::endl;
            }
            if(globalDerivatives.cols()) {
                ...
                std::cout << std::endl;
                std::cout << "  Global Derivatives:" << globalDerivatives.format(CleanFmt) << std::endl;
            }
        }
    }
