...
namespace gbl {
    ...
    bool GblTrajectory::isValid() const { return constructOK; }

    /// Retrieve number of point from trajectory
    unsigned int GblTrajectory::getNumPoints() const { return numAllPoints; }
    ...
    void GblTrajectory::construct() {
        ...
        if(numAllPoints < 2) {
            std::cout << " GblTrajectory construction failed: too few GblPoints " << std::endl;
            return;
        }
        ...
        for(unsigned int iTraj = 0; iTraj < numTrajectories; ++iTraj) {
            ...
            for(itPoint = thePoints[iTraj].begin(); itPoint < thePoints[iTraj].end(); ++itPoint) {
                for(itMeas = itPoint->getMeasBegin(); itMeas < itPoint->getMeasEnd(); ++itMeas) {
                    if(itMeas->isEnabled()) {
    ...
    void GblTrajectory::defineOffsets() {
        ...
        for(unsigned int iTraj = 0; iTraj < numTrajectories; ++iTraj) {
            ...
            std::vector<GblPoint>::iterator itPoint;
            for(itPoint = thePoints[iTraj].begin() + 1; itPoint < thePoints[iTraj].end() - 1; ++itPoint) {
    ...
    void GblTrajectory::calcJacobians() {
        ...
        for(unsigned int iTraj = 0; iTraj < numTrajectories; ++iTraj) {
            ...
            std::vector<GblPoint>::iterator itPoint;
            for(itPoint = thePoints[iTraj].begin() + 1; itPoint < thePoints[iTraj].end(); ++itPoint) {
                if(numStep == 0) {
            ...
            for(itPoint = thePoints[iTraj].end() - 1; itPoint > thePoints[iTraj].begin(); --itPoint) {
                if(itPoint->getOffset() >= 0) {
                    scatJacobian = itPoint->getP2pJacobian();
     ...
     */
    unsigned int GblTrajectory::getResults(int aSignedLabel, Eigen::VectorXd& localPar, Eigen::MatrixXd& localCov) const {
        if(not fitOK)
            return 1;
        std::pair<std::vector<unsigned int>, MatrixXd> indexAndJacobian = getJacobian(aSignedLabel);
        ...
        localCov = indexAndJacobian.second * aMat * indexAndJacobian.second.adjoint();
        return 0;
    }
    ...
    unsigned int GblTrajectory::getMeasResults(unsigned int aLabel,
        ...
        for(unsigned int i = 0; i < numData; ++i) {
            getResAndErr(
                firstData + i, (aLabel != skippedMeasLabel), aResiduals(i), aMeasErrors(i), aResErrors(i), aDownWeights(i));
        }
        return 0;
    }
    ...
    unsigned int GblTrajectory::getScatResults(unsigned int aLabel,
        ...
        for(unsigned int i = 0; i < numData; ++i) {
            getResAndErr(firstData + i, true, aResiduals(i), aMeasErrors(i), aResErrors(i), aDownWeights(i));
        }
        return 0;
    }
     ...
     * \return error code (non-zero if trajectory not fitted successfully)
     */
    unsigned int GblTrajectory::getResults(int aSignedLabel, TVectorD& localPar, TMatrixDSym& localCov) const {
        if(not fitOK)
            return 1;
        std::pair<std::vector<unsigned int>, MatrixXd> indexAndJacobian = getJacobian(aSignedLabel);
        unsigned int nParBrl = indexAndJacobian.first.size();
        ...
        MatrixXd aLocalCov = indexAndJacobian.second * aMat * indexAndJacobian.second.adjoint();
        // convert to ROOT
     ...
     * \return error code (non-zero if trajectory not valid (constructed successfully))
     */
    unsigned int GblTrajectory::getLabels(std::vector<std::vector<unsigned int>>& aLabelList) const {
        if(not constructOK)
            return 1;
     ...
     */
    void GblTrajectory::getResAndErr(
        ...
        double* derLocal;
        theData[aData].getResidual(aResidual, aMeasVar, aDownWeight, numLocal, indLocal, derLocal);
     ...
     * \param [out] aMeasError Error of Measurement
     */
    void GblTrajectory::getResAndErr(unsigned int aData, double& aResidual, double& aMeasError) {
     ...
     * constraint) \exception 11 : inner transformation matrix with too few columns (must be >= number of rows) \exception 12
     * : inner transformation matrices with varying sizes \exception 13 : too many external derivatives (must be <= number of
     * columns of inner transformation matrix)
     */
    void GblTrajectory::prepare() {
        unsigned int nDim = theDimension.size();
        // upper limit
        unsigned int maxData = numMeasurements + nDim * (numOffsets - 2) + externalSeed.rows();
        ...
        // limit the scope of proDer:
        {
            ...
            Eigen::Matrix<double, Eigen::Dynamic, 5, Eigen::ColMajor /* default */, 5, 5> proDer;
            // loop over trajectories
            for(unsigned int iTraj = 0; iTraj < numTrajectories; ++iTraj) {
                for(itPoint = thePoints[iTraj].begin(); itPoint < thePoints[iTraj].end(); ++itPoint) {
                    ...
                    if(itPoint->numMeasurements()) {
                        for(itMeas = itPoint->getMeasBegin(); itMeas < itPoint->getMeasEnd(); ++itMeas) {
                            ...
                            maxNumGlobals = std::max(maxNumGlobals, itMeas->getNumGlobals());
                            MatrixXd transDer;
                            ...
                            getFitToLocalJacobian(labDer, matDer, *itPoint, measDim, nJacobian);
                            if(measDim > 2) {
                                matPDer = matP * matDer;
                                    ...
                                    if(labDer[ilabel] > 0) {
                                        ...
                                        if(ifirst > ilast) {
                                            ...
                                            for(unsigned int k = iOff; k < 5; ++k) {
                                                proDer(k - iOff, ifirst) = matPDer(k, ilabel);
                                            }
                                        }
                                    }
                                    ++ilabel;
                            ...
                            for(unsigned int i = iOff; i < 5; ++i) {
                                if(aPrec(i) > minPrecision) {
                                    ...
                                    aData.addDerivatives(i, labDer, matPDer, iOff, localDer, numLocals, transDer);
        ...
        // external seed
        if(externalPoint) {
            std::pair<std::vector<unsigned int>, MatrixXd> indexAndJacobian = getJacobian(externalPoint);
        ...
        if(nExt > 0) {
            ...
            if(nExtDer > nInnerTransCols) {
                std::cout << " GblTrajectory::prepare external measurement with too many derivatives: " << nExtDer
            ...
            for(unsigned int iExt = 0; iExt < nExt; ++iExt) {
                ...
                GblData aData(1U, ExternalMeasurement, externalMeasurements(iExt), externalPrecisions(iExt));
     ...
     */
    unsigned int
    GblTrajectory::fit(double& Chi2, int& Ndf, double& lostWeight, const std::string& optionList, unsigned int aLabel) {
        ...
        try {
            ...
            for(unsigned int i = 0; i < theData.size(); ++i) {
                // skipped (internal) measurement ?
                if(theData[i].getLabel() == skippedMeasLabel && theData[i].getType() == InternalMeasurement)
                    continue;
    ...
    void GblTrajectory::milleOut(MilleBinary& aMille) {
        ...
        std::vector<GblData>::iterator itData;
        for(itData = theData.begin(); itData != theData.end(); ++itData) {
            itData->getAllData(aValue, aErr, numLocal, labLocal, derLocal, aTraj, aPoint, aMeas, aRow);
            if(itData->getType() == InternalMeasurement)
                thePoints[aTraj][aPoint].getGlobalLabelsAndDerivatives(aMeas, aRow, labGlobal, derGlobal);
            else
                labGlobal.resize(0);
            aMille.addData(aValue, aErr, numLocal, labLocal, derLocal, labGlobal, derGlobal);
        }
        aMille.writeRecord();
    }
    ...
    void GblTrajectory::printTrajectory(unsigned int level) const {
        ...
        std::cout << " Number of GblPoints          : " << numAllPoints << std::endl;
        std::cout << " Number of points with offsets: " << numOffsets << std::endl;
        std::cout << " Number of fit parameters     : " << numParameters << std::endl;
        std::cout << " Number of measurements       : " << numMeasurements << std::endl;
        if(externalMeasurements.rows()) {
            std::cout << " Number of ext. measurements  : " << externalMeasurements.rows() << std::endl;
        }
        if(externalPoint) {
            std::cout << " Label of point with ext. seed: " << externalPoint << std::endl;
        }
        if(constructOK) {
        ...
        if(level > 0) {
            ...
            if(fitOK) {
                ...
                theVector.print();
                std::cout << "  Covariance matrix (bordered band part):" << std::endl;
