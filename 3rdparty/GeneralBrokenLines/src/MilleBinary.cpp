...
namespace gbl {
     ...
     */
    MilleBinary::MilleBinary(const std::string& fileName, bool doublePrec, bool keepZeros, unsigned int aSize)
        : binaryFile(fileName.c_str(), std::ios::binary | std::ios::out), intBuffer(), floatBuffer(), doubleBuffer(),
          doublePrecision(doublePrec), globalDerKeepZeros(keepZeros) {
    ...
    MilleBinary::~MilleBinary() { binaryFile.close(); }
     ...
     */
    void MilleBinary::addData(double aMeas,
                              double aErr,
    ...
    void MilleBinary::writeRecord() {
        ...
        const int recordLength = (doublePrecision) ? -intBuffer.size() * 2 : intBuffer.size() * 2;
        binaryFile.write(reinterpret_cast<const char*>(&recordLength), sizeof(recordLength));
        if(doublePrecision)
            binaryFile.write(reinterpret_cast<char*>(&doubleBuffer[0]), doubleBuffer.size() * sizeof(doubleBuffer[0]));
        else
            binaryFile.write(reinterpret_cast<char*>(&floatBuffer[0]), floatBuffer.size() * sizeof(floatBuffer[0]));
        binaryFile.write(reinterpret_cast<char*>(&intBuffer[0]), intBuffer.size() * sizeof(intBuffer[0]));
