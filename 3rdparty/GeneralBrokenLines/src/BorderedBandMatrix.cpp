...
namespace gbl {
        ...
        for(unsigned int i = 0; i < anIndex->size(); ++i) {
            ...
            for(unsigned int j = 0; j <= i; ++j) {
                ...
                if(iIndex < nBorder) {
                    ...
                    theMixed(jIndex, iIndex - nBorder) += (*aVector)[i] * aWeight * (*aVector)[j];
                } else {
                    unsigned int nBand = iIndex - jIndex;
                    theBand(nBand, jIndex - nBorder) += (*aVector)[i] * aWeight * (*aVector)[j];
    ...
    void BorderedBandMatrix::solveAndInvertBorderedBand(const VVector& aRightHandSide, VVector& aSolution) {
        ...
        if(numBorder > 0) { // need to use block matrix decomposition to solve
            ...
            const VVector auxVec =
                aRightHandSide.getVec(numBorder) - auxMat * aRightHandSide.getVec(numCol, numBorder); // = b1 - Xt*b2
            ...
            // solve for band part
            const VVector bandSolution = solveBand(aRightHandSide.getVec(numCol, numBorder)); // = x
    ...
    VMatrix BorderedBandMatrix::solveBand(const VMatrix& aRightHandSide) const {
        ...
        for(unsigned int iBorder = 0; iBorder < numBorder; iBorder++) {
            ...
            {
                for(int j = 1; j < std::min(nRow, nCol - i); ++j) {
                    aSolution(iBorder, j + i) -= theBand(j, i) * aSolution(iBorder, i);
                }
            }
            for(int i = nCol - 1; i >= 0; i--) // backward substitution
            {
        ...
        for(int i = nCol - 1; i >= 0; i--) {
            ...
            for(int j = i; j >= std::max(0, i - nRow + 1); j--) {
                for(int k = j + 1; k < std::min(nCol, j + nRow); ++k) {
                    rxw -= inverseBand(abs(i - k), std::min(i, k)) * theBand(k - j, j);
                }
