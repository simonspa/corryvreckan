...
namespace gbl {
    ...
    VMatrix::VMatrix(const VMatrix& aMatrix) : numRows(aMatrix.numRows), numCols(aMatrix.numCols), theVec(aMatrix.theVec) {}
     ...
     */
    unsigned int VMatrix::getNumRows() const { return numRows; }

    /// Get number of columns.
    /**
     * \return Number of columns.
     */
    unsigned int VMatrix::getNumCols() const { return numCols; }
    ...
    void VMatrix::print() const {
        ...
        for(unsigned int i = 0; i < numRows; ++i) {
            for(unsigned int j = 0; j < numCols; ++j) {
                if(j % 5 == 0) {
                    std::cout << std::endl
                              << std::setw(4) << i << "," << std::setw(4) << j << "-" << std::setw(4)
                              << std::min(j + 4, numCols) << ":";
                }
    ...
    VSymMatrix::VSymMatrix(const unsigned int nRows) : numRows(nRows), theVec((nRows * nRows + nRows) / 2) {}
     ...
     * \return Number of rows.
     */
    unsigned int VSymMatrix::getNumRows() const { return numRows; }
    ...
    void VSymMatrix::print() const {
        ...
        for(unsigned int i = 0; i < numRows; ++i) {
            for(unsigned int j = 0; j <= i; ++j) {
                if(j % 5 == 0) {
                    std::cout << std::endl
                              << std::setw(4) << i << "," << std::setw(4) << j << "-" << std::setw(4) << std::min(j + 4, i)
                              << ":";
                }
    ...
    VVector::VVector(const unsigned int nRows) : numRows(nRows), theVec(nRows) {}

    VVector::VVector(const VVector& aVector) : numRows(aVector.numRows), theVec(aVector.theVec) {}
    ...
    void VVector::putVec(const VVector& aVector, unsigned int start) {
        std::memcpy(&theVec[start], &aVector.theVec[0], sizeof(double) * aVector.numRows);
    }
     ...
     * \return Number of rows.
     */
    unsigned int VVector::getNumRows() const { return numRows; }
    ...
    void VVector::print() const {
        ...
        for(unsigned int i = 0; i < numRows; ++i) {
            ...
            if(i % 5 == 0) {
                std::cout << std::endl << std::setw(4) << i << "-" << std::setw(4) << std::min(i + 4, numRows) << ":";
            }
